...
from ConceptFingerprint.asizeof import asized, asizeof
from ConceptFingerprint.Classifier.hoeffding_tree_shap import \
    HoeffdingTreeSHAPClassifier
from ConceptFingerprint.Classifier.meta_info_classifier import DSClassifier
from ConceptFingerprint.Data.load_data import (AbruptDriftStream,
...
def process_option(option):
    ...
    def get_discrimination_results(log):
        ...
        model_changes = log['active_model'] != log['active_model'].shift(
    ...
    monitoring_header = ('example', 'y', 'p', 'is_correct', 'right_sum', 'wrong_sum', 'overall_accuracy', 'active_model', 'ground_truth_concept', 'drift_occured', 'change_detected', 'model_evolution',
                         'active_state_active_similarity', 'active_state_buffered_similarity', 'all_state_buffered_similarity', 'all_state_active_similarity', 'feature_weights', 'concept_probs', 'cpu_time', 'ram_use', 'fingerprint_ram')
    logging.info(option)
    ...
    for i in tqdm.tqdm(range(option['length']), position=mp_id+1, desc=f"Worker {mp_id} {str(UID)[:4]}...", leave=False):
        ...
        if buffered_data is not None:
            ...
            concept_similarities = [
                (int(k), v) for k, v in buffered_similarities.items() if k != 'active']
        ...
        if weights is not None and option['save_feature_weights']:
            observation_monitoring['feature_weights'] = ';'.join(
                [f"{s}{f}:{v}" for s, f, v in weights])
        else:
            observation_monitoring['feature_weights'] = -1

        concept_probs = classifier.monitor_concept_probabilities
        if concept_probs is not None:
            observation_monitoring['concept_probs'] = ';'.join(
                [f"{cid}:{v}" for cid, v in concept_probs.items()])
        else:
            observation_monitoring['concept_probs'] = -1
        ...
        if all_state_active_similarity is not None:
            ...
            concept_similarities = [
                (int(k), v) for k, v in active_similarities.items() if k != 'active']
            concept_similarities.sort(key=lambda x: x[0])
...
if __name__ == "__main__":
    ...
    my_parser.add_argument(
        '--fsmethod', default="default", type=str, nargs="*")
    ...
    my_parser.add_argument(
        '--min_sim_stdev', default=0.01, nargs="*", type=float)
    my_parser.add_argument(
        '--max_sim_stdev', default=0.1, nargs="*", type=float)
    my_parser.add_argument(
        '--buffer_ratio', default=0.25, nargs="*", type=float)
    ...
    my_parser.add_argument('--ifeatures', default=['IMF', 'MI'], nargs="*",
    ...
    synthetic_dist = ['SigNoiseGenerator-1', 'SigNoiseGenerator-2', 'SigNoiseGenerator-3', 'SigNoiseGenerator-4', 'SigNoiseGenerator-5', 'SigNoiseGenerator-6', 'SigNoiseGenerator-7', 'SigNoiseGenerator-8', 'SigNoiseGenerator-9', 'SigNoiseGenerator-10', 'FeatureWeightExpGenerator',
    ...
    if given_options:
        ...
        for ds_name, ds_type in dataset_options:
            ...
            for seed in seeds:
                if not args.experimentopts:
                    for sim_opt, MIcalc, fs_opt, fingerprint_opt, fingerprint_bins_opt, ws_opt, sim_gap_opt, fp_gap_opt, na_fp_gap_opt, ob_gap_opt, sim_std_opt, min_sim_opt, max_sim_opt, br_opt, drift_width_opt in classifier_options:
                        option = {
                    ...
                    for fingerprint_bins_opt, ws_opt, sim_gap_opt, fp_gap_opt, na_fp_gap_opt, ob_gap_opt, sim_std_opt, min_sim_opt, max_sim_opt, br_opt, drift_width_opt in classifier_options:
    ...
    if args.single:
        ...
        pool = mp.Pool(processes=args.cpu, initializer=tqdm.tqdm.set_lock, initargs=(
            tqdm.tqdm.get_lock(),))
        run_files = []
        pool_run = tqdm.tqdm(pool.imap(func=process_option, iterable=option_set, chunksize=1), total=len(
            option_set), position=0, desc="Experiment", leave=True)

...
def concept_normal(rs, im, isd, isk, ik):
    return rs.normal(loc=im, scale=isd)
...
class Sampler():
    def __init__(self, num_features, random_state = None, signal_noise_ratio=0.5):
        self.num_features = num_features
        self.random_state = check_random_state(random_state)
        self.signal_noise_ratio = signal_noise_ratio
        self.feature_data = []
        self.intra_mean = self.random_state.rand()
        self.intra_stdev = 0.5
        self.noise_num_bins = 2
        self.signal_num_bins = num_features
        self.signal_threshold = self.num_features * self.signal_noise_ratio
        ...
        for i in range(self.num_features):
            ...
            feature_index = i
            num_bins = self.signal_num_bins if i < self.signal_threshold else self.noise_num_bins
            num_concepts_in_bin = 10 / num_bins
            bin_range = [0, 100]
            bin_width = (bin_range[1] - bin_range[0]) / num_bins
            bins = [bin_range[0] + bin_width * j for j in range(num_bins)]
            bins.append(bin_range[1])
            chosen_bin = int(concept_index // num_concepts_in_bin)
            bin_start = bins[chosen_bin]
            bin_end = bins[chosen_bin + 1]
            bin_mid = (bin_start + bin_end) / 2
            chosen_mean = bin_mid

            data['mean'] = chosen_mean
            # data['stdev'] = 0.01 + (self.num_features - i) * 0.25
            data['stdev'] = 7.5
...
class SigNoiseGenerator(Stream):
    ...
    def __init__(self, tree_random_state=None, sample_random_state=None, sampler_random_state=None,
                 signal_noise_ratio=0.5, n_classes=2, n_cat_features=0,
    ...
    def generate_sampler(self):
        ...
        """
        self.sampler = Sampler(self.n_num_features, self.sampler_random_state, self.signal_noise_ratio)
